#include "CTSearcher.h"
#include <iomanip>
#include <algorithm>

CTSearcher::CTSearcher( sc_module_name n): sc_module( n ), t_skt("t_skt"), base_offset(0),
	ctb_searcher_l("ctb_searcher_l"), ctb_searcher_r("ctb_searcher_r"), ct_searcher_b("ct_searcher_b")
{
    SC_THREAD(searchCT);
    t_skt.register_b_transport(this, &CTSearcher::blocking_transport);
...
void CTSearcher::blocking_transport(tlm::tlm_generic_payload &payload,
    ...
    case tlm::TLM_READ_COMMAND:
        ...
        break;
        case CTSEARCHER_CHECK_ADDR:
            tmp = std::min(o_r_idx.num_available(), o_q_idx.num_available());
        ...
        break;
        default:
        std::cerr << "Error! CTSearcher::blocking_transport: address 0x"
        ...
        case CTSEARCHER_R_ADDR:
            for (int i = 0; i < POINT*MAX_HULL_SIZE/8; ++i) {
                if (mask_ptr[i] == 0xff) {
                    ...
                    std::cerr << "Error! CTSearcher::blocking_transport: missing p point with index: "
                                << i / (POINT/8) << std::endl;
                }
                if (mask_ptr[i+POINT*MAX_HULL_SIZE/8] == 0xff) {
                    _q.range((i+1)*8-1, (i)*8) = data_ptr[i+POINT*MAX_HULL_SIZE/8];
                } else {
                    std::cerr << "Error! CTSearcher::blocking_transport: missing q point with index: "
                                << (i) / (POINT/8) << std::endl;
                }
            }
            i_r.write(_r);
